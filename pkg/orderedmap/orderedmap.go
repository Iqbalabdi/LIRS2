...
func (om *OrderedMap) MoveToSpecificIndex(keyNode interface{}, flagNode interface{}) (ok bool) {
	var moved *node
	var subs *node
	// Remove from current position
	anode, ok := om.table[keyNode]
	bnode, ok := om.table[flagNode]
	if !ok {
		return false
	}
	...
	moved.Prev = bnode.Prev
	moved.Next = bnode
	bnode.Prev.Next = moved
	subs.Prev = moved
